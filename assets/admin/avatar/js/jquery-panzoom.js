...
(function( $ ){
  ...
  var methods = {
    ...
    'crop': function() {
		...
		width= data.target_dimensions.x;
		height= data.target_dimensions.y;	
		if(width > 0 && height > 0){		
			if(width > height) {	
				new_height = 202;
		...
		for(i=0;i<=50;i++){
			if(i==0){
				zoom[i]= {};
					zoom[i].x1= data.position.x1;
					zoom[i].x2= data.position.x2;
					zoom[i].y1= data.position.y1;
					zoom[i].y2= data.position.y2;								
			}else{	
				zoom[i]= {};
					zoom[i].x1= zoom[i-1].x1*1 - steps.zoom.x;
					zoom[i].x2= zoom[i-1].x2*1 + steps.zoom.x;
					zoom[i].y1= zoom[i-1].y1*1 - steps.zoom.y;
					zoom[i].y2= zoom[i-1].y2*1 + steps.zoom.y;
			}
		}				
    ...
    'dragComplete': function() {
		...
		var $this= this;
		setTimeout(function(){			
			if(data.position.y1 > 42){
				$this.animate({
					top: 42
				}, 150);			
			}	
			
			if(data.position.y2 < 240){
				margin= 240 - data.position.y2;
				$this.animate({
					top: data.position.y1 + margin + 3
				}, 150);			
			}	
		
			if(data.position.x1 > 10){
				$this.animate({
					left: 8
				}, 150, function(){
					data.position.x1= 8;					
			...
			if(data.position.x2 < 210){
				margin= 210 - data.position.x2;
				$this.animate({
					left: data.position.x1 + margin
				}, 150);					
			}	
