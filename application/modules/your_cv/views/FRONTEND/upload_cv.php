    ...
    init: {
        ...
        FileUploaded: function(up, file, info) {
            ...
            try {
                   ...
                   title = (lang == 'vi' ? 'Upload xảy ra lỗi' : 'Upload error occurred');
                   showFileError(title);
                }    
        },
        Error: function(up, err) {
            ...
            if(err.code=='-600'){
                title = (lang == 'vi' ? 'Kích cỡ file quá lớn' : 'File size too big');
                showFileError(title); 
            }
            else if(err.code=='-601'){
                title = (lang == 'vi' ? "File không đúng định dạng" : "File  isn't format");
                showFileError(title);   
            }
            else if(err.code=='-200'){
                title = (lang == 'vi' ? 'Không thể upload' : 'Can not upload');
                showFileError(title);
            }
				...
				<div class="upload_cv">
					...
					<div class="form-profile">
			            <form class="form-horizontal" role="form" id="frmUploadCV">
			                ...
			                <div class="form-group" >
			                    <div class="lable"><?=lang('attach_file')?></div>
			                    <div class="ip_file left">   
                              <input type="hidden" id="uploaded_id" value="" />
          									<div class="input file">
          										<input type="text" id="file-name" value="<?=lang('attach_file')?>" readonly="true" />
          									</div>
          			               <input type="button" id="upload_file" class="button-browse left" value="BROWSE"/> (doc,docx,pdf) < 4MB
          			              <p>You can only have 1 CV attached. This action will replace your former attached CV with this one</p>
                          </div>
          								<div class="clearfix"></div>
			                </div>
			                ...
			                <div class="form-group">
			                    <div class="lable"><?=lang('verification')?></div>
			                ...
			                <div class="form-group">
								...
								<div class="mt8 mbt20 btn-form"><input type="button" value="<?=lang('send')?>" id="btSend" class="btn-update" /></div>
								<div class="clearfix"></div>
			                </div>
			            </form>
				    </div>
