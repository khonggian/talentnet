...
class Admin extends MX_Controller {
	private $module = 'who_download_brochure';
	public $table= 'who_download_brochure';
	...
	function index(){
		...
		/* if(!empty($result)){
			foreach($result as $r){
				...
				$table_category = $r->category == 'wz_information' ? INFORMATION_CATEGORY_TB : NEW_ARRIVALS_CATEGORY_TB;
        ...
        if($this->input->get('excel')!='' && $this->input->get('excel')){
    		...
    		$this->template->write('title', 'Manage Download');
	...
	function filter_category(){
		...
		$news_category = $this->model->fetch('*',DOWNLOAD_BROCHURE_TB,"`status` = 1 ","id","asc");
