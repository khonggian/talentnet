...
class Admin_model extends MY_Model {
	...
	function getList($limit=-1, $page=-1, $get= array()){
		...
		$this->db->from(CAREER_CATEGORY_TB." AS subscribe_tb");
		
		if(!empty($get['keyword']))
		{
			$k= $get['keyword'];
			$this->db->where("subscribe_tb.name_vi LIKE '%{$k}%' OR subscribe_tb.name_en LIKE '%{$k}%' OR subscribe_tb.description_vi LIKE '%{$k}%' OR subscribe_tb.description_en LIKE '%{$k}%'");
		}
		...
		if(!empty($get['range']))
		{
		   ...
		   */  
		   ...
		   {
		    $this->db->order_by('subscribe_tb.order', 'asc');
		   }
