<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
class Admin_model extends MY_Model {
	private $table = 'career';
	
	function __construct(){
		parent::__construct();
		$this->table = PREFIX.$this->table;
	}	
	
	function getList($limit=-1, $page=-1, $get= array()){
		if($limit == -1)
		{
			$this->db->select('count(*) as sum');
		}
		else
		{
			$this->db->select("career_tb.*, career_category_tb.name_en as name_category");
			//EXPORT EXCEL
			if(empty($get['excel']))
			{
				$this->db->limit($limit, $page);
			}
		}
				
		$this->db->from(CAREER_TB." AS career_tb");
		$this->db->join(CAREER_CATEGORY_TB." AS career_category_tb", "career_tb.function = career_category_tb.id");
		
		if(!empty($get['keyword']))
		{
			$k= $get['keyword'];
			$this->db->where("career_tb.name_vi LIKE '%{$k}%' OR career_tb.name_en LIKE '%{$k}%' OR career_category_tb.description_vi LIKE '%{$k}%' OR career_category_tb.description_en LIKE '%{$k}%'");
		}

		if(!empty($get['range']))
		{
			$range= 	explode('-', $get['range']);
			$start= 	date('Y-m-d', strtotime($range[0])).' 00:00:00';
			$end=		date('Y-m-d', strtotime($range[1])).' 23:59:59';
			$this->db->where("career_tb.created >= '".$start. "' AND career_tb.created <= '".$end."'");
		}

		$query = $this->db->get();


		if($query->result())
		{
			if($limit == -1)
			{
				return $query->row()->sum;
			}
			else
			{
				return $query->result();
			}
		}
		else
		{
			return false;
		}			
	}
	
	function save($nid, $mid, $data, $data_field, &$type){
		$module= $this->get('*', ADMIN_MODULE_TB, "id = '{$mid}'");
		$module_table= 			PREFIX.$module->folder;
		$data['changed']= 		NOW;	
		if(empty($nid))
		{
			$data['created']= NOW;
			$data['work_location'] = trim($this->input->post("pid"));
			$data['level'] = trim($this->input->post("level"));
			$data['function'] = trim($this->input->post("function"));
			if($this->db->insert($module_table, $data)){
				$nid= $this->db->insert_id();
			}
		}
		else
		{
			$data['work_location'] = $this->input->post("pid");
			$data['level'] = trim($this->input->post("level"));
			$data['function'] = trim($this->input->post("function"));
			$this->db->update($module_table, $data, "id = '{$nid}'");
		}		
		$this->adminwz_model->formFunction($nid, $module, $data, $data_field, $type);
		$this->session->unset_userdata('referrer_page_list');
	}
}
?>